...
namespace FwksLabs.Libs.Infra.Observability.Options;
...
public record ObservabilityOptions
{
    ...
    private void ConfigureOtlpExporterOptions(OtlpExporterOptions options, string url)
    {
        if (CollectorEndpoint is null || Uri.IsWellFormedUriString(CollectorEndpoint, UriKind.Absolute) is false)
            throw new AppConfigurationException($"{CollectorEndpoint} is missing or misconfigured. Check the value and try again.");
