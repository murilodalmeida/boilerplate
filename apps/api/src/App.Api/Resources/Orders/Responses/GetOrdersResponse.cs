...
{
    public static PagedResponse<GetOrdersResponse> Transform(PagedResult<OrderEntity> page)
    {
        var items = page.Items.Select(x => new GetOrdersResponse(
            x.Id.Encode(), x.CustomerId.Encode(), x.CreationDate.Humanize(), x.PaymentDate.Humanize(), x.Products, x.Total)).ToList();

        return new(page.PageNumber, page.PageSize, items.Count, items);
    }
}